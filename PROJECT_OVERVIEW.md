...
- **Безопасность при сбоях:** файл конфигурации заменяется атомарно через временный; при ошибке исходный файл остаётся нетронутым.
...
- **vcpkg** — только для `zet017config` (подтянет `libxml2` автоматически)
...
# --- XML-модуль (vcpkg подтянет libxml2) ---
