 ...
 * равный NULL, просто исключает свою часть работы. Симметрично устроены и
 * данные: при чтении отсутствующий элемент оставляет поле структуры
 * нетронутым, при записи отсутствующий элемент файла не создаётся, а всё,
 * чем модуль не владеет, переносится в новый файл как есть.
 *
 ...
 * zet017tcp наружу не отдаёт, поэтому модуль не пытается такой файл
 * сочинить: чтение перечисленное выше берёт, запись перечисленное выше
 * обновляет, остальное остаётся в файле нетронутым.
 */
 ...
 * начинает отбивать совершенно нормальный документ. Подставляем разделитель
 * текущей локали и разбираем как обычно.
 *
 * Локаль процесса нам не принадлежит — со статическим рантаймом (/MT) у DLL
...
/* Найти <Device serial="..."> с нужным серийным номером и заодно проверить
 * шапку документа. Общая часть чтения и записи.
 *
 ...
 * zet017_device_set_config, иначе в файл попадёт не то, что уйдёт в
 * устройство. Модуль намеренно независим от zet017tcp и вызвать оригинал не
 * может, поэтому при изменении таблиц там их нужно поправить и здесь —
 * ровно как продублированные структуры в zet017config.h.
 * -------------------------------------------------------------------------- */
 ...
 * snprintf уважает LC_NUMERIC: под русской локалью он выдал бы "1,5",
 * и такой файл перестал бы читаться — strtod при разборе ждёт точку. */
...
static int zet017_cfg_build_path(char* buf, size_t size, const char* dir_name, const char* file_name)
{
    ...
    /* Разделитель не нужен, если каталог уже им заканчивается. Проверяем оба
     * варианта: на Windows в путях встречаются и '\', и '/'. */
 ...
 * же отступом, что и у соседей, чтобы не ломать вёрстку файла. Все значения
 * здесь числовые, спецсимволов XML в них не бывает, поэтому экранирование
 * не требуется. */
static void zet017_cfg_set_child_text(xmlNodePtr parent, const char* name, const char* text)
{
     ...
     * встраиваемся ПЕРЕД ним, иначе новый элемент окажется за ним.
     *
     * Сначала элемент, только потом отступ: xmlAddPrevSibling сливает
     * соседние текстовые узлы, и отступ, добавленный перед хвостовым
     * пробельным узлом, просто растворился бы в нём. Перед элементом
     * сливаться не с чем. */
...
/* Заменить первые count значений в списке через запятую, сохранив хвост.
 *
 * Нужно для <KodAmplify>: в реальном devices.cfg это 32 позиции, а модуль
 * владеет только первыми восемью — остальные принадлежат устройству и
 * должны вернуться в файл нетронутыми.
 *
    ...
    const struct zet017_cfg_config* config, const struct zet017_cfg_tenso_config* tenso_config)
{
    ...
    /* Приоритет как в zet017_device_set_config: заданная частота перебивает
     * сырой код. <Freq> — та же величина в герцах; в файле она лежит рядом с
     * <ModaADC>, и оставить её от прежней настройки значило бы отдать ZETLAB
     * внутренне противоречивый файл. <Rate> не трогаем: аналога в структурах
     * нет и вычислить его не из чего. */
    ...
    for (xmlNodePtr ch = channels->children; ch != NULL; ch = ch->next) {
         ...
         * ZETLAB не является — такой элемент оставляем как был. */
...
/* Набран ли файл переводами строки в стиле Windows (CRLF).
 *
 * libxml2 по требованию стандарта XML сворачивает CRLF в одиночный LF ещё на
 * разборе, а ZETLAB пишет devices.cfg по-виндовому. Без восстановления
 * исходного варианта сохранение переписывало бы КАЖДУЮ строку файла.
 * Смотрим на первый перевод строки — он в объявлении XML, то есть заведомо
 * в начале файла.
 *
 * Заодно это проверка, что файл вообще открывается: -1 означает «донора нет»,
 * и тогда до libxml2 дело не доходит — иначе на каждую такую попытку она
 * писала бы в stderr предупреждение о ненайденной сущности. */
 ...
 *   - LF разворачивается обратно в CRLF, если файл был набран по-виндовому;
 *   - пустой тег закрывается как "<AFCH />", а не "<AFCH/>" (libxml2 пробел
 *     не хранит, а ZETLAB его ставит).
 *
 * И то и другое — чистая косметика, сам XML от этого не меняется. Но без них
 * каждое сохранение переписывало бы десятки строк, которых никто не трогал,
 * и diff файла перестал бы что-либо показывать.
 *
    ...
    const struct zet017_cfg_config* config, const struct zet017_cfg_tenso_config* tenso_config)
{
    ...
    /* Файл-донор обязателен: элементы, которыми модуль не владеет, взять
     * больше неоткуда. Стиль переводов строки снимаем ДО разбора — парсер их
     * нормализует. */
    ...
    /* Пишем во временный файл рядом и подменяем им исходный: донор прочитан
     * из этого же файла, и прямая запись при сбое уничтожила бы единственную
     * копию конфигурации. */
    ...
    /* Выгружаем в память без переформатирования: отступы, порядок элементов и
     * незнакомые модулю секции переносятся ровно так, как их разобрал парсер.
     *
     * Кодировку указываем ЯВНО. У ZETLAB объявление XML идёт без неё, и тогда
     ...
     * превращается в "ZET 058 &#x2116;...". Формально это тот же документ, но
     * читать его человеком становится нельзя, да и простой разборщик на
     * стороне ZETLAB может не справиться. С encoding="UTF-8" кириллица
