 ...
 * неопределённым. Поэтому объявляем нужные типы вручную — заголовок
 ...
 *   -2  — не удалось открыть/распарсить файл;
 ...
 * служебной структуре, которую zet017tcp наружу не отдаёт, поэтому собрать
 * корректный файл «с нуля» из zet017_cfg_config невозможно. Функция
 * обновляет только те элементы, которые сама же умеет читать, а весь
 * остальной файл переносит без изменений. Если файла нет — возвращается -2.
 *
 * Записываются:
 ...
 *
 * Приоритет значений совпадает с zet017_device_set_config: человеческая
 * величина перебивает сырой код, поэтому в файл попадает ровно то, что
 * ушло бы в устройство:
 ...
 * Нулевое поле означает «не задано», тогда берётся сырой код (moda_adc,
 ...
 *                  секция <Channels> не трогается вовсе).
 *
 * Файл заменяется целиком через временный файл рядом с ним, поэтому при
 * сбое записи исходный файл остаётся нетронутым.
 *
