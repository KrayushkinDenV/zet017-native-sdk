 ...
 * неопределёнными. Поэтому нужные типы объявлены вручную — заголовок
 ...
 * <windows.h> намеренно НЕ подключается в публичный заголовок,
 * чтобы избежать конфликтов имён с макросами WinAPI.
 * WINAPI = __stdcall — определяется напрямую. */
#if defined(_WIN32)
 ...
 *
 * Если файл открыть не удалось, сервер создаётся успешно, а лог молча
 * отключается — логирование не должно нарушать инициализацию. Чтобы получить
...
/* Сменить лог-файл во время работы. Новый путь открывается до закрытия
 * старого, поэтому при неудаче прежнее состояние сохраняется. NULL или "" —
 * выключить файловый лог (и освободить файл). Потокобезопасно.
 * Коды возврата: 0 успех, -1 невалидный handle,
 ...
 * считываются под ОДНИМ захватом внутреннего мьютекса, то есть из одного и
 * того же окна. При поканальном чтении мьютекс отпускается между вызовами,
 * и поток приёма успевает перезаписать начало окна — тогда первый канал
 * прочитан до перезаписи, а последний уже после. Для расчётов, где каналы
 * сравниваются между собой (измерительный и опорный), это обязательное
 * условие.
 *
 ...
 *     плоский 1D Array of Single, раскладку выполняет вызывающий.
 *
 ...
 *
 * Параметры:
 *   pointer   — позиция, ОТ которой выполняется чтение назад (отсчётов
 *               на канал);
 ...
 *               не может определить размер предоставленного ей буфера,
 *               поэтому без этого параметра ошибка вызывающего означала
 *               бы запись за его границы. Требуется
 *               capacity >= channels * size;
 *   channels  — сюда записывается число активных каналов. Можно передать
 *               NULL. Заполняется и при отказе -7 тоже, чтобы вызывающий
 *               мог определить требуемый размер буфера без подбора.
 *
